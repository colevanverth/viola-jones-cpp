...
void queuePool(Pool<int> &pool, VJLearner &learner, std::mutex &fMutex,
               std::vector<cv::Point> &facePoints, cv::Mat &frame);
...
void faceRectangle(cv::Mat &frame, int mPos, int nPos) {
  ...
  for (int m = 0; m < SUB_WINDOW_SIZE; m++) {
    for (int n = 0; n < BORDER_WIDTH; n++) {
      frame.at<cv::Vec3b>(m + mPos, n + nPos + SUB_WINDOW_SIZE - BORDER_WIDTH) =
          COLOR;
    }
  }
  // Top horizontal bar.
  ...
  for (int n = 0; n < SUB_WINDOW_SIZE; n++) {
    for (int m = 0; m < BORDER_WIDTH; m++) {
      frame.at<cv::Vec3b>(m + mPos + SUB_WINDOW_SIZE - BORDER_WIDTH, n + nPos) =
          COLOR;
    }
  }
}
