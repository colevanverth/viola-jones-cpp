...
void WLearner::train(const std::vector<IntegralImage> &imgs,
  ...
  for (auto &w : wavelets) {
    pool.queue(
        [&]() { return this->m_bestSplit(w, imgs, imgWeights, targets); });
  }
  std::vector<WErrorInfo> infos = pool.getReturnVals();

  WErrorInfo bestInfo;
  bestInfo = *std::min_element(infos.begin(), infos.end(),
                               [](const WErrorInfo &i1, const WErrorInfo &i2) {
...
WLearner::WErrorInfo
WLearner::m_bestSplit(const Wavelet &w, const std::vector<IntegralImage> &imgs,
                      const std::vector<float> &imgWeights,
                      const std::vector<Prediction> &targets) {
  ...
  std::sort(splitInfos.begin(), splitInfos.end(),
            [](const WSplitInfo &w1, const WSplitInfo &w2) {
  ...
  for (int i = 1; i < splitInfos.size(); i++) {
    faceError.push_back(faceError.back() +
                        ((splitInfos[i].prediction == Prediction::FACE)
                             ? splitInfos[i].imgWeight
                             : 0));
  }
  for (int i = splitInfos.size() - 1; i >= 1; i--) {
    nonFaceError.push_back(nonFaceError.back() +
                           ((splitInfos[i].prediction == Prediction::NON_FACE)
                                ? splitInfos[i].imgWeight
                                : 0));
  }
...
WLearner::predict(const std::vector<IntegralImage> &imgs) {
...
float WLearner::m_error(const Wavelet &w, const Wavelet::waveVal wVal,
...
float WLearner::error(const std::vector<IntegralImage> &imgs,
                      const std::vector<float> &imgWeights,
                      const std::vector<Prediction> &targets) {
  return this->m_error(this->m_wavelet, this->m_splitVal, imgs, imgWeights,
                       targets);
}

void to_json(json &j, const WLearner &wL) {
  j = json{{"m_splitVal", wL.m_splitVal}, {"m_wavelet", wL.m_wavelet}};
}
